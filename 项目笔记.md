...
## Pycharm的mac苹果芯片专业版安装

首先需要在电脑上准备两样：1.安装包。2.激活工具。
...
然后我们双击打开安装包，在弹出的对话框中将软件拖入application，等待安装完成，去程序坞中找到安装好的软件直接双击打开，然后会弹出这样的对话框。
...
选择OK。这时候等待程序加载完成，这期间不需要操控程序。

然后我们将刚才的激活工具进行解压，解压的位置随意，因为激活成功后可以删除该文件。解压完成后需要记住包括该文件在内的文件位置。
...
```
Success! Activate pycharm to 2099
```
...
项目目录下的git终端输入下方代码，可以从远程仓库拉取最新的代码合并到我当前的所在分支，在运行前需要确保已经设置了远程仓库并且有权限访问。
